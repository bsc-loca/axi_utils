...
module axiu_id_remap #(
    parameter SLV_AXI_ID_WIDTH = 0,
    parameter MST_AXI_ID_WIDTH = 0
) (
    input clk,
    input arstn,
    input [MST_AXI_ID_WIDTH-1:0] slv2mst_table[2**SLV_AXI_ID_WIDTH],
    input [SLV_AXI_ID_WIDTH-1:0] mst2slv_table[2**MST_AXI_ID_WIDTH],
    AXI_BUS.Slave slv,
    AXI_BUS.Master mst
);

    assign mst.ar_id = slv2mst_table[slv.ar_id];
    assign mst.aw_id = slv2mst_table[slv.aw_id];
    assign slv.r_id = mst2slv_table[mst.r_id];
    assign slv.b_id = mst2slv_table[mst.b_id];
